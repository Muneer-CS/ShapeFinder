...
The Twelve Data `/stocks` catalog currently supplies symbol, name, exchange, MIC, country, type, currency, FIGI/CFI, and restricted identifier placeholders, but no reliable listing, first-trade, or delisting date. ShapeFinder therefore does not invent historical eligibility dates or add an external listing source. The normalized provider-neutral record retains only the fields needed for membership; historical usefulness is learned from local coverage and range-specific outcomes.

Candidate ordering first places symbols with overlapping partial cache coverage, then applies a stable symbol hash within the partial and uncached groups. This finishes likely-small gaps first while spreading new work across the universe instead of concentrating attempts in an alphabetical cluster of newly listed instruments. A process-local cursor keyed by universe, range, interval, and reference length advances transient attempts, while scan-ready symbols are always skipped. Successful bars and coverage are durable in SQLite. Migration 3 records hydration failures: `no_data` suppresses identical or at least 50%-overlapping historical ranges for seven days without affecting unrelated future ranges; unsupported-symbol and hard provider-rejection outcomes cool down for seven days; fetched-but-insufficient histories cool down for the exact range for one day. Rate limits, daily quota exhaustion, timeouts, and transient provider failures are not persistently suppressed. Per-context async locks prevent concurrent identical searches from duplicating hydration work in one process.

Hydration diagnostics log only symbol, interval, requested boundaries, normalized category, and safe provider status/code—never the API key, request URL, or raw response. API statistics preserve the original attempt/success fields and add candidates considered, historical-ineligible skips (currently always zero because the catalog lacks dates), cooldown skips, partial-cache priorities, fetched, persisted, newly-ready, useful-success rate, category counts, and daily-quota state.
