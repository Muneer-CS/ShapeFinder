...
Candidate ordering is a stable symbol hash, spreading each bounded batch across the universe instead of concentrating attempts in an alphabetical cluster of newly listed instruments. A process-local cursor keyed by universe, range, interval, and reference length advances transient attempts, while scan-ready symbols are always skipped. Successful bars and coverage are durable in SQLite. Migration 3 also records range-specific permanent hydration failures: `no_data`, unsupported-symbol, and hard provider-rejection outcomes cool down for seven days, while fetched-but-insufficient histories cool down for one day. Rate limits, daily quota exhaustion, timeouts, and transient provider failures are not persistently suppressed. Per-context async locks prevent concurrent identical searches from duplicating hydration work in one process.

Hydration diagnostics log only symbol, interval, requested boundaries, normalized category, and safe provider status/code—never the API key, request URL, or raw response. API statistics preserve the original attempt/success fields and add fetched, persisted, newly-ready, suppressed, category-count, and daily-quota fields so fetch success is not confused with scan readiness.
